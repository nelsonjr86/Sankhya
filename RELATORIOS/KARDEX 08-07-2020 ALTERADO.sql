...
TGFCAB.NUMNOTA 'NOTA',
CASE TGFCAB.TIPMOV WHEN 'V' THEN 'VENDA' WHEN 'D' THEN 'DEVOLUCAO_VENDA' WHEN 'E' THEN 'DEVOLUCAO_COMPRA' ELSE 'COMPRA' END 'TIPO',
...
(SELECT ISNULL(SUM(I.ATUALESTOQUE * I.QTDNEG),0) FROM TGFITE I INNER JOIN TGFCAB C ON (C.NUNOTA=I.NUNOTA) WHERE I.CODPROD=ITE.CODPROD AND I.ATUALESTOQUE <> 0 AND C.DTNEG < :PERIODO.INI AND C.CODEMP = TGFCAB.CODEMP AND C.TIPMOV IN ('V','C','D','E')) +
...
AND ITE.CODLOCALORIG = LO.CODLOCAL
 ...
 AND ISNULL(TGFCAB.DTENTSAI,TGFCAB.DTNEG) >= :PERIODO.INI ORDER BY ISNULL(TGFCAB.DTENTSAI,TGFCAB.DTNEG), ITE.ATUALESTOQUE DESC, TGFCAB.NUNOTA