...
FROM TSIEMP E INNER JOIN TFPFUN F ON (E.CODEMP = F.CODEMP)
		...
		LEFT JOIN TFPOCO O ON (F.CODFUNC = O.CODFUNC AND O.CODHISTOCOR IN (29,22,18,19) AND CONVERT(date, R.DTALTER) = CONVERT(date, O.DTALTER))
...
AND (R.EXTRA + R.EXTRANOT + R.EXCEDENTE + R.DOMFER + R.DOMFERNOT) > 0
AND F.DTDEM IS NULL 
AND :TIPO = 'E'
)
...
AND (R.EXCEDENTENOT + R.NOTURNA) > 0
AND F.DTDEM IS NULL 
AND :TIPO = 'N'
)
