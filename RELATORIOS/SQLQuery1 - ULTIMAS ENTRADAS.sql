SELECT /*ULTIMAS ENTRADAS*/
* FROM (
	SELECT row_number() OVER (ORDER BY PRODUTO ASC,DATA_ENTRADA DESC,NUNOTA DESC) AS row_number, *
	FROM (

		SELECT
			PRO.REFERENCIA AS 'PRODUTO',
			PRO.DESCRPROD AS 'DESCRICAO',
			PRO.COMPLDESC AS 'COMPLEMENTO',
			CAB.NUNOTA AS 'NUNOTA',
			RTRIM(LTRIM(PAR.NOMEPARC)) AS 'FORNECEDOR',
			CAB.DTENTSAI AS 'DATA_ENTRADA',
			CAB.NUMNOTA AS 'NOTA',
			ITE.QTDNEG AS 'QTDE',
			ITE.VLRUNIT AS 'VLR_UNIT',
			ALIQIPI AS 'ALIQ_IPI',
			CASE WHEN ITE.VLRSUBST = 0 THEN 0 WHEN ITE.VLRSUBST >0 THEN (ITE.VLRSUBST/ITE.QTDNEG) END  AS 'ST',
 			CASE WHEN CAB.VLRFRETE = 0 THEN 0 WHEN CAB.VLRFRETE > 0 THEN ((CAB.VLRFRETE*100)/CAB.VLRNOTA) END AS 'PERC_FRETE',
			CASE WHEN CAB.VLRFRETE > 0 THEN (ITE.VLRUNIT+((ITE.VLRUNIT+ITE.VLRUNIT*(ALIQIPI*0.01))*((CAB.VLRFRETE/CAB.VLRNOTA)))+ITE.VLRUNIT*(ALIQIPI*0.01))
			WHEN (ITE.VLRUNIT+((ITE.VLRUNIT+ITE.VLRUNIT*(ALIQIPI*0.01)))+ITE.VLRUNIT*(ALIQIPI*0.01)) > 0 THEN (ITE.VLRUNIT+ITE.VLRUNIT*(ALIQIPI*0.01))
			ELSE ITE.VLRUNIT END AS 'VALOR_TOTAL',

			CASE WHEN ALIQIPI IS NULL AND CAB.VLRFRETE = 0 THEN ITE.VLRUNIT 
			WHEN CAB.VLRFRETE > 0 AND ITE.VLRSUBST > 0 THEN (ITE.VLRSUBST/ITE.QTDNEG)+(ITE.VLRUNIT+((ITE.VLRUNIT+ITE.VLRUNIT*(ALIQIPI*0.01))*((CAB.VLRFRETE/CAB.VLRNOTA)))+ITE.VLRUNIT*(ALIQIPI*0.01))
			WHEN CAB.VLRFRETE > 0 AND ALIQIPI > 0 THEN ITE.VLRUNIT+(ITE.VLRUNIT*(ALIQIPI*0.01))
			WHEN ITE.VLRSUBST = 0  THEN (ITE.VLRUNIT+((ITE.VLRUNIT+ITE.VLRUNIT*(ALIQIPI*0.01))*((CAB.VLRFRETE/CAB.VLRNOTA)))+ITE.VLRUNIT*(ALIQIPI*0.01)) 
			WHEN ITE.VLRSUBST > 0 THEN (ITE.VLRSUBST/ITE.QTDNEG)+(ITE.VLRUNIT+((ITE.VLRUNIT+ITE.VLRUNIT*(ALIQIPI*0.01))*((CAB.VLRFRETE/CAB.VLRNOTA)))+ITE.VLRUNIT*(ALIQIPI*0.01))  END AS 'ST_VALOR_TOTAL'
								
		FROM TGFCAB CAB
		INNER JOIN TGFITE ITE ON (ITE.NUNOTA = CAB.NUNOTA)
		INNER JOIN TGFPAR PAR ON (PAR.CODPARC = CAB.CODPARC)
		INNER JOIN TGFPRO PRO ON (PRO.CODPROD = ITE.CODPROD)

		WHERE CAB.TIPMOV = 'C'
		AND CAB.STATUSNOTA = 'L'
		AND CAB.CODEMP = :EMPRESA
		AND (
			(LEN(:GRUPO) > 4 AND PRO.REFERENCIA LIKE :GRUPO) OR
			(LEN(:GRUPO) <= 4 AND PRO.REFERENCIA LIKE :GRUPO+'%')
		)
		AND (
			(:GERENCIAL = 'S' AND CAB.CODVEND <> 59 AND CAB.NUNOTA IN (SELECT NUNOTA FROM TGFFIN WHERE ORIGEM = 'E' AND CODTIPTIT <> 69)) OR :GERENCIAL = 'N'
		)

	) SUB
) TabelaNumerada
WHERE row_number BETWEEN 0 AND 100

