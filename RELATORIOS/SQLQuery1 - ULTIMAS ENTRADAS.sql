SELECT * FROM (
	...
	FROM (
		...
		SELECT
			...
			CASE WHEN ITE.VLRSUBST = 0 THEN 0 WHEN ITE.VLRSUBST > 0 THEN (ITE.VLRSUBST/ITE.QTDNEG)+(ITE.VLRUNIT+((ITE.VLRUNIT+ITE.VLRUNIT*(ALIQIPI*0.01))*((CAB.VLRFRETE/CAB.VLRNOTA)))+ITE.VLRUNIT*(ALIQIPI*0.01)) END AS 'ST_VALOR_TOTAL'
