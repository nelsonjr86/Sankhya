...
* FROM (
	...
	FROM (
		...
		SELECT
			...
			ALIQIPI AS 'ALIQ_IPI',
			CASE WHEN DIN.AD_DIFST IS NULL THEN (ITE.VLRSUBST/ITE.QTDNEG) WHEN DIN.AD_DIFST  IS NOT NULL THEN ((ITE.VLRSUBST/ITE.QTDNEG)+(DIN.AD_DIFST/ITE.QTDNEG)) END  AS 'ST',
			CASE WHEN CAB.VLRFRETE = 0 THEN 0 WHEN CAB.VLRFRETE > 0 THEN ((CAB.VLRFRETE*100)/CAB.VLRNOTA) END AS 'PERC_FRETE',
			CASE WHEN CAB.VLRFRETE > 0 THEN (ITE.VLRUNIT+((ITE.VLRUNIT+ITE.VLRUNIT*(ALIQIPI*0.01))*((CAB.VLRFRETE/CAB.VLRNOTA)))+ITE.VLRUNIT*(ALIQIPI*0.01))
			...
			ELSE ITE.VLRUNIT END AS 'TOTAL_SEM_ST',
			CASE WHEN (ITE.VLRSUBST) > 0 AND DIN.AD_DIFST >0 THEN 0 WHEN (DIN.AD_DIFST) IS NULL THEN ((ITE.VLRSUBST/ITE.QTDNEG))+(ITE.VLRUNIT+((ITE.VLRUNIT+ITE.VLRUNIT*(ALIQIPI*0.01))*((CAB.VLRFRETE/CAB.VLRNOTA)))+ITE.VLRUNIT*(ALIQIPI*0.01))
			WHEN (DIN.AD_DIFST) IS NOT NULL THEN ((ITE.VLRSUBST/ITE.QTDNEG)+(DIN.AD_DIFST/ITE.QTDNEG))+(ITE.VLRUNIT+((ITE.VLRUNIT+ITE.VLRUNIT*(ALIQIPI*0.01))*((CAB.VLRFRETE/CAB.VLRNOTA)))+ITE.VLRUNIT*(ALIQIPI*0.01)) END AS 'TOTAL_COM_ST'
			
			
		FROM TGFCAB CAB
		INNER JOIN TGFITE ITE ON (ITE.NUNOTA = CAB.NUNOTA)
		INNER JOIN TGFDIN DIN ON (DIN.NUNOTA = CAB.NUNOTA)
